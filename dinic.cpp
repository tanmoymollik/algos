...
struct Dinic {
   ...
   Dinic(int n) : nodes(n), inf(numeric_limits<T>::max() / 4) {
      memset(nodeEnd, -1, n * sizeof(int));
   }
   void addEdge(int u, int v, T cap = 1) {
      edgeList.PB(Edge<T> (v, nodeEnd[u], cap));
      nodeEnd[u] = (int)edgeList.SZ-1;
      edgeList.PB(Edge<T> (u, nodeEnd[v], 0));
   ...
   T maxFlow(int src, int snk) {
      //REP(i, edgeList.SZ) edgeList[i].flow = 0;
      source = src, sink = snk;
      T tot = 0;
